...
import { fetchMutation, fetchQuery } from '@my/backend/nextjs';
import { FunctionArgs, FunctionReference } from '@my/backend/server';
...
export const ConvexAdapter: Adapter = {
  ...
  async getUserByEmail(email) {
    return maybeUserFromDB(await callQuery(api.authAdapter.getUserByEmail, { email }));
  },
...
function maybeVerificationTokenFromDB(verificationToken: Doc<'verificationTokens'> | null) {
...
function toDB<T extends object>(
  ...
  [K in keyof T]: T[K] extends Date ? number : null extends T[K] ? undefined : T[K];
} {
  const result: any = {};
  for (const key in obj) {
    const value = obj[key];
    result[key] = value instanceof Date ? value.getTime() : value === null ? undefined : value;
  }
  return result;
}
