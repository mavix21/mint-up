import { YStack, XStack, Chip, Paragraph, withStaticProperties } from '@my/ui';
...
} from '../../entities/event-intentions/constants';
 ...
 */
function EventIntentionsSelectorList({ children }: { children?: React.ReactNode }) {
  return (
 ...
 */
function EventIntentionsOption({ intention, showDescription = false }: EventIntentionsOptionProps) {
  ...
  return (
    <YStack gap="$1">
      <Chip
        size="$3"
        pressable
        onPress={() => onToggle(intention)}
        backgroundColor={selected ? '$blue9' : '$color5'}
        borderColor={selected ? '$blue10' : 'transparent'}
        opacity={disabled ? 0.5 : 1}
        hoverStyle={{
          backgroundColor: selected ? '$blue10' : '$color6',
        }}
        pressStyle={{
          backgroundColor: selected ? '$blue10' : '$color6',
        }}
      ...
      {showDescription && (
        <Paragraph fontSize="$2" color="$color10" maxWidth={150}>
 ...
 */
function EventIntentionsDefaultOptions({ showDescription = false }: { showDescription?: boolean }) {
  return (
    <>
...
function EventIntentionsHelperText({ children }: EventIntentionsHelperTextProps) {
 ...
 * ```
 */
export const EventIntentionsSelector = withStaticProperties(EventIntentionsSelectorRoot, {
